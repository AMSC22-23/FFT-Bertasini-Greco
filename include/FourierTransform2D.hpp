...
class FourierTransform2D : public Transform<cv::Mat> {
    ...
    FT ft;
    public:
    class InputSpace : public Transform<cv::Mat>::InputSpace {
        public:
        cv::Mat data;
        InputSpace(const cv::Mat& og_image);
        auto get_data() const -> cv::Mat override;
    };
    class OutputSpace : public Transform<cv::Mat>::OutputSpace {
        public:
    ...
    auto get_input_space(const cv::Mat& og_image) const -> std::unique_ptr<Transform<cv::Mat>::InputSpace> override;
    auto get_output_space() const -> std::unique_ptr<Transform<cv::Mat>::OutputSpace> override ;
    auto operator()(Transform<cv::Mat>::InputSpace& in, Transform<cv::Mat>::OutputSpace& out, bool inverse) const -> void override;
};
